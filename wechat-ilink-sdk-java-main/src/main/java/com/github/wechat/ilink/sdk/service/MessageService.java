...
public class MessageService {
    ...
    public void sendText(LoginContext loginContext, String toUserId, String text) throws IOException {
        sendText(loginContext, toUserId, text, null);
    }

    /**
     * 发送文本。
     *
     * @param contextTokenOverride 优先使用该 context_token（来自触发回复的那条入站消息）；
     *     为 null 时回退到会话缓存的最新 token。
     */
    public void sendText(
        LoginContext loginContext, String toUserId, String text, String contextTokenOverride)
        throws IOException {
    ...
    public void sendFile(
        ...
        sendFile(
            loginContext, toUserId, fileBytes, fileName, caption, null);
    }
    ...
    public void sendFile(
        ...
        if (caption != null && !caption.isEmpty()) {
            sendText(loginContext, toUserId, caption, contextTokenOverride);
        }
