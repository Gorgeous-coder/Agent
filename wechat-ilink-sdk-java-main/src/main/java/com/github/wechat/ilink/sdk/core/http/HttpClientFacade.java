...
/**
 * HTTP client facade used by the SDK. The {@link RequestBody#create(MediaType, String)} and
 * {@link RequestBody#create(MediaType, byte[])} factories are deprecated in OkHttp 4.x but remain
 * the only Java-friendly way to build request bodies without relying on Kotlin extension
 * functions. The deprecation is suppressed at the class level because the replacement
 * ({@code String.toRequestBody(mediaType)}) is not callable from Java directly.
 */
