...
public class UpdateService {
  ...
  public List<WeixinMessage> poll(LoginContext loginContext) throws IOException {
    ...
    for (WeixinMessage msg : msgs) {
      // 只缓存"用户主动发来的消息"的 context_token。
      // getupdates 会把 bot 自己发出的消息（message_type=2）也回捞，
      // 若用它们的 token 覆盖缓存，后续发送会带上错误 token，导致消息无法投递。
      if (msg.getMessage_type() != null && msg.getMessage_type().intValue() == 2) {
        continue;
      }
