...
本项目语音消息收发依赖 **ffmpeg** 做音频转码（微信 silk ⇄ mp3/wav）。

> ⚠️ **ffmpeg 体积巨大（单个 exe 约 140MB，解压后 400MB+），已从 git 仓库中排除**（见 `.gitignore` 的 `tools/`）。克隆/拉取代码后**不会自带 ffmpeg**，需要手动下载放置，否则语音相关功能（TTS 语音回复、语音消息转文本）会报「Cannot run program ffmpeg」。

### 下载步骤（Windows 64 位）

1. 访问官方构建站：[gyan.dev ffmpeg builds](https://www.gyan.dev/ffmpeg/builds/)（或 [BtbN 镜像](https://github.com/BtbN/FFmpeg-Builds/releases)），下载 **release-essentials** 或 **release-full** 的 `ffmpeg-master-latest-win64-gpl.zip`（约 170MB）。
   - 建议选 **essentials** 版（体积更小，转码功能已足够；full 版体积大一倍但功能更多）。
2. 解压得到 `ffmpeg-master-latest-win64-gpl` 目录，将**整个目录**放到项目根下的 `tools/` 目录：
   ```
   <项目根>/tools/ffmpeg-master-latest-win64-gpl/bin/ffmpeg.exe
   <项目根>/tools/ffmpeg-master-latest-win64-gpl/bin/ffprobe.exe
   <项目根>/tools/ffmpeg-master-latest-win64-gpl/bin/ffplay.exe
   ```
   （`tools/` 目录若不存在则手动新建。）
3. 校验：
   ```bash
   tools/ffmpeg-master-latest-win64-gpl/bin/ffmpeg.exe -version
   ```
   能看到版本号即成功。

### 配置 ffmpeg 路径

在 `src/main/resources/ai-bot.properties` 中指定（或留空让程序自动探测）：

```properties
ai.ffmpeg-path=tools/ffmpeg-master-latest-win64-gpl/bin/ffmpeg.exe
```

- 留空时程序会尝试从 `PATH` 环境变量寻找 `ffmpeg`。
- 协作成员拉取代码后**无需任何 git 操作**，只需按上述步骤放置 ffmpeg 即可正常运行语音功能。

### 为什么不在仓库里带 ffmpeg？

- GitHub 单文件限制 100MB（LFS 除外），ffmpeg.exe 约 140MB 无法直接提交。
- 不使用 Git LFS（团队不便统一购买额度/配置）。
- 因此采用「代码仓库 + 本地手动放置」的方式，ffmpeg 属于运行时依赖而非代码，不入库更合理。
