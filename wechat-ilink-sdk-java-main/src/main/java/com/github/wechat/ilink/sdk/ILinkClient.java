...
public class ILinkClient implements AutoCloseable {
    ...
    /**
     * 发送文本，可显式指定 context_token（来自触发回复的那条入站消息）。
     * 不传（null）时使用会话缓存的最新 token。
     */
    public void sendTextWithContext(String toUserId, String text, String contextToken)
        throws IOException {
        messageService.sendText(requireLogin(), toUserId, text, contextToken);
    }
    ...
    /**
     * 发送文件，可显式指定 context_token（来自触发回复的那条入站消息）。
     */
    public void sendFileWithContext(
        String toUserId, byte[] fileBytes, String fileName, String caption, String contextToken)
        throws IOException {
        messageService.sendFile(
            requireLogin(), toUserId, fileBytes, fileName, caption, contextToken);
    }
